...
namespace DataLayer.Repository
{
    public static class RepositoryFactory
    {
        ...
        public static IRepository<T> CreateRepository<T>() where T : class
        {
            ...
            else if (typeof(T) == typeof(Examination))
            {
                return new ExaminationRepository(new PatientManagerContext()) as IRepository<T> ?? throw invalid_repo_exception;
            }
            else if (typeof(T) == typeof(Perescription))
            {
                return new PerescriptionRepository(new PatientManagerContext()) as IRepository<T> ?? throw invalid_repo_exception;
            }
